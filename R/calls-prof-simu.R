...
# ?????????????? Pour quoi le zoning initial est toujour le même ?????????????
plotZ(Z) #only zones # plot Z calls "dispZ.R"
# more detailed plot
valRef=quantile(map$krigGrid,na.rm=TRUE,prob=c(0.4,0.7)) # derive the values correpsonding to the given quantile
...
# profvis : This function will run an R expression with profiling, 
# and then return an htmlwidget for interactively exploring the profiling data.
# 
...
# (remove, i.e. merge into englobing zone, r grow - if grow, isolated zone -> grows bigger but remains isolated from others (distIsoZ parameter) - non isolated zone -> joins closest zone with same labels
################################################################
p=profvis({criti=correctionTree(c(0.4,0.7),map,pErr=0.9,optiCrit=2,minSize=0.012,minSizeNG=1e-3,distIsoZ=0.075,simplitol=1e-3,LEQ=5,MAXP=0.1,LASTPASS=TRUE,disp=0,SAVE=TRUE,ONE=FALSE)
})# argument SAVE will keep results at all branches in zf,zk,critere,mdist
p # 
...
plotZ(zk[[2]][[1]]$zonePolygone) # result of removal of zone 7
# or
K=zk[[2]][[1]]
Z=K$zonePolygone
