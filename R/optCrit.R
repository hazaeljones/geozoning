...
#' @examples
#' # not run
optCrit=function(seed,map,Cmin=0,f=1,disp=1)
#########################################################
{
# make seed data
...
critAdj=as.matrix(unlist(critAdj))
cost=as.matrix(unlist(cost))
nq=as.matrix(unlist(nq))
pu=as.matrix(pu)
C=as.matrix(unlist(C))
mat=cbind(C,pu,critAdj,crit,cost,mqm,nq)
mode(mat)="numeric"
rownames(mat)=NULL
colnames(mat)=c("C","p","critAdj","crit","cost",paste("q",1:5,sep=""),"nq")
if(disp) plot(lab,pu,ylim=c(2,14),main=paste("seed=",seed,"-Maximize adj. crit",sep=""),xlab="")
