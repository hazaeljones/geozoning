...
```
This vignette reads an existing zoning, that was not done with geozoning, in order to calculate its quality criterion. It uses 2 objects from the geozoning package.
The shape1 object contains the zoning, it is a SpatialPolygonsDataFrame, where each SpatialPolygons is a zone. It was read from a shapefile, available in the extdata directory.
Yield contains yield data, imported from a csv file, also available in the extdata directory.
